...
- `packages/shared` auth DTO、错误码、密码策略
...
如果现在开始正式开发，建议按下面顺序开工：

1. 补 `M0`：Drizzle、测试基线、CI 脚本、环境加载。
2. 启动 `S1-1`：先做 schema 和 `/auth/*` 契约。
3. 完成邮箱密码登录最小闭环。
4. 再逐步补 `SSO discovery`、`pending`、`invite`、`MFA` 和 `audit`。
