...
- `S1-2` 角色、显式 allow/deny、用户直授和授权优先级判定
...
- `R5-B` `S1-2` RBAC 规则收口
  - 落角色模型、显式 allow/deny 和用户直授例外授权
  - 固化授权优先级与失效时间边界
  - 为后续管理后台授权编辑能力准备真实数据结构
...
| R5 | `S1-2` RBAC 持久化 | 分两步完成：`R5-A` 先落工作台目录与群组授权来源，`R5-B` 再补角色/deny/user grant | `R5-A` 已完成，`R5-B` 激活中 |
| R6 | `S1-3` 启动链路 | 从工作台进入真实应用/会话创建 | 工作台按钮不再只是准备态提示 |
...
- `S1-2` 仍未完成角色体系、显式 deny 优先级和用户直授例外授权。
