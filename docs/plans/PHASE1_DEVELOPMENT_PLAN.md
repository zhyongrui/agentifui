...
- better-auth 接入
- 数据持久化和真实认证逻辑
- `S1-3` 的真实授权来源、群组上下文后端、配额服务和启动链路
- MFA（TOTP）
- 登录/登出审计
- CI 细化
...
1. 先补登录/登出/失败登录审计，把当前认证链路的关键事件固定下来。
2. 然后补邀请创建端和 MFA（TOTP）。
3. 再把当前内存认证服务替换为 `better-auth + db` 持久化实现。
4. 最后做 `S1-1` 回归，把剩余 AC 串成可验证闭环。
