...
- `R1` `S1-3` Gateway-backed workspace catalog
  - 已将 `/apps` 从前端假数据切到真实 `GET /workspace/apps`
  - 已补 gateway route tests、web client tests、类型检查和 HTTP 验证

下一个激活项：

- `R2` `S1-1` 审计事件最小闭环
  - 先覆盖登录成功、登录失败、登出三类事件
  - 先用内存审计存储冻结事件模型、错误边界和查询接口
  - 完成后再进入 `MFA (TOTP)`，不并发散开
...
| R4 | `S1-1` 持久化认证 | 将当前内存认证替换为 `better-auth + db` 持久化实现 | 认证主链路脱离内存态 |
