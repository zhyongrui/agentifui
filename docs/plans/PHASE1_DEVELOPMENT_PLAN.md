...
- better-auth 接入
- `S1-3` 的真实授权持久化来源、启动链路和真实会话创建
- `S1-2` RBAC 的持久化来源和授权判定收口
- CI 细化
...
- `R4-A` `S1-1` DB 持久化认证底座
  - 已接入 `packages/db` runtime、迁移目录和重置脚本
  - 已将默认 Gateway 认证、会话、MFA、邀请和审计链路切到 PostgreSQL 持久化实现
  - 已让登出真正撤销会话，而不再只是返回成功响应
  - 已补跨重启持久化集成测试、全量测试、构建验证和真实 HTTP 冒烟

下一个激活项：

- `R4-B` `S1-1` better-auth 接入收口
  - 在已落地的 DB 持久化边界上接入 better-auth
  - 保持现有 `/auth/*` 路由契约和前端交互不回退
  - 完成后再进入 `R5` RBAC 持久化来源
...
| R4 | `S1-1` 持久化认证 | 分两步完成：`R4-A` 先替换为 `db` 持久化，`R4-B` 再接入 `better-auth` | 认证主链路脱离内存态，better-auth 开始接管认证内核 |
| R5 | `S1-2` RBAC 持久化 | 固化角色、群组成员和授权规则来源 | 应用授权不再依赖内存映射 |
...
- `S1-3` 仍未完成真实启动链路、持久化授权来源和真实配额服务。
