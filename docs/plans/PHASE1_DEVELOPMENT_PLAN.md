...
- `R12-B` `S2` 分享与协作
  - 在已完成的 attachment / run replay 边界之上补 conversation 分享记录、创建/撤销接口和受分享访问边界
  - 先做最小只读分享和组内访问控制，再补分享审计和更细的协作行为
  - 继续以现有 conversation/run 持久化边界作为分享、历史和后续 timeline 的基础
...
| R11 | `S3-2` 审计合规深化 | 补审计导出、PII 标记、剩余 run-aware 合规能力 | 已完成 |
| R12 | `S2` 残余 backlog | 文件上传、分享、细粒度执行时间线与历史衔接 | 已完成 `R12-A` 附件主链路，继续推进分享 / timeline / history |
