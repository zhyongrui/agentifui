...
test('admin source management feeds retrieval-backed chat citations', async ({ page }) => {
...
Residents may request approved late access for labs.`);
  ...
  await expect(page.getByText(`Queued ${sourceTitle} for ingestion.`)).toBeVisible();

  const sourceCard = page.locator('article.admin-app-card').filter({
    has: page.getByRole('heading', { name: sourceTitle }),
  });
  await expect(sourceCard).toBeVisible();
  await expect(sourceCard.getByText('strategy markdown_sections')).toBeVisible();
  ...
  const switchPolicyWatchButton = appCard(page, 'Policy Watch').getByRole('button', {
  ...
  await expect(
    runHistoryPanel.locator('.artifact-link-card').getByText(sourceTitle, {
      exact: true,
    }),
  ).toBeVisible();
});
