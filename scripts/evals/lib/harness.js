...
function resolveGoldenPath(fixtureId) {
    return join(GOLDEN_DIR, `${fixtureId}.json`);
}
function resolveIncidentPath(fixtureId) {
...
async function injectJson(app, input) {
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
        throw new Error(`${input.method} ${input.url} failed with ${response.statusCode}: ${JSON.stringify(body)}`);
    }
    return body;
}
function resolveActorIdentity(fixture, actorId) {
    const actor = fixture.actors?.[actorId];
    const displayName = actor?.displayName ??
        (actorId === "admin" ? "Eval Tenant Admin" : "Eval Workspace User");
    const emailLocalPart = actor?.emailLocalPart ?? (actorId === "admin" ? "admin.eval" : "user.eval");
    return {
        displayName,
        email: `${sanitizeToken(emailLocalPart)}-${sanitizeToken(fixture.id)}@example.net`,
    };
}
async function registerAndLoginActor(app, fixture, actorId) {
...
async function launchConversation(app, sessionToken, fixture) {
...
async function fetchConversation(app, sessionToken, conversationId) {
    return injectJson(app, {
        method: "GET",
...
async function fetchRun(app, sessionToken, runId) {
    return injectJson(app, {
        method: "GET",
...
function buildEvalRequestMessages(conversation, nextPrompt) {
...
function buildEvalInputs(fixture, stepIndex, runtimeInput) {
    return {
        eval: {
...
async function applyKnowledgeSourceSetup(app, adminSessionToken, fixture) {
...
async function applyToolRegistrySetup(app, adminSessionToken, fixture) {
...
function toPendingActionRequest(step) {
...
function extractLatestAssistantText(run) {
...
function sanitizeSnapshotString(value) {
    return value
...
function sanitizeSnapshotValue(value) {
    if (typeof value === "string") {
        return sanitizeSnapshotString(value);
    }
    if (Array.isArray(value)) {
        return value.map((entry) => sanitizeSnapshotValue(entry));
    }
...
function normalizeRun(fixture, prompt, run) {
    const evalInput = typeof run.inputs.variables === "object" &&
        run.inputs.variables !== null &&
        typeof Reflect.get(run.inputs.variables, "eval") === "object" &&
        Reflect.get(run.inputs.variables, "eval") !== null
        ? Reflect.get(run.inputs.variables, "eval")
        : null;
    return {
        appId: run.app.id,
        status: run.status,
        ...
        runtime: run.runtime
            ...
            : null,
        versions: {
            fixtureVersion: typeof evalInput?.fixtureVersion === "string"
                ? evalInput.fixtureVersion
                : fixture.fixtureVersion,
            promptVersion: typeof evalInput?.promptVersion === "string"
                ? evalInput.promptVersion
                : fixture.promptVersion,
            runtimeVersion: typeof evalInput?.runtimeVersion === "string"
...
function normalizeSnapshot(fixture, prompt, conversation, run) {
...
function diffValues(actual, expected, path = "$") {
    ...
    if (typeof actual === "object" &&
        ...
        return keys.flatMap((key) => diffValues(actualRecord[key], expectedRecord[key], `${path}.${key}`));
    }
    return [
        {
            path,
...
async function runEvalFixture(fixture) {
    ...
    try {
        ...
        const launch = await launchConversation(app, actorSessions[launchActor].sessionToken, fixture);
        const conversationId = launch.data.conversationId;
        let runId = launch.data.runId ?? "";
        let traceId = launch.data.traceId ?? "";
        let prompt = "";
        let currentConversation = (await fetchConversation(app, actorSessions[launchActor].sessionToken, conversationId)).data;
        for (let stepIndex = 0; stepIndex < fixture.steps.length; stepIndex += 1) {
            ...
            if (step.kind === "completion") {
                prompt = step.message;
                const completion = await injectJson(app, {
                    method: "POST",
                ...
                const pendingActions = await injectJson(app, {
                    method: "GET",
                ...
                const pendingItem = pendingActions.data.items.find((item) => item.status === "pending");
                if (!pendingItem) {
            ...
            currentConversation = (await fetchConversation(app, actor.sessionToken, conversationId)).data;
        }
        if (!runId) {
        ...
        const golden = readJsonFile(goldenPath);
...
function renderEvalMarkdownReport(collection) {
    const lines = [
...
function writeEvalOutputs(outputDir, collection) {
    ensureDirectory(outputDir);
    writeJsonFile(join(outputDir, "results.json"), collection);
    writeFileSync(join(outputDir, "comparison-report.md"), renderEvalMarkdownReport(collection), "utf8");
}
export async function runEvalFixtures(options) {
    ...
    if (options.failOnDiff) {
        ...
        if (failures.length > 0) {
            throw new Error(`Eval comparison failed for ${failures.map((entry) => entry.fixture.id).join(", ")}.`);
        }
    }
    return collection;
}
async function runReleaseAuthSmoke(app) {
    const identity = {
...
async function runReleaseAdminSmoke(app) {
    const admin = await registerAndLoginActor(app, EVAL_FIXTURES.find((fixture) => fixture.id === "tenant-control-approval"), "admin");
    await injectJson(app, {
        method: "GET",
        url: "/admin/users",
        headers: {
...
async function runReleaseChatSmoke(app) {
...
export async function runReleaseGate(options) {
    ...
    try {
        ...
        if (options.outputDir) {
            ...
            writeFileSync(join(options.outputDir, "release-gate.md"), renderReleaseGateMarkdown(report), "utf8");
        }
...
export function loadSavedIncident(fixtureId) {
    return readJsonFile(resolveIncidentPath(fixtureId));
}
export function printEvalSummary(collection) {
