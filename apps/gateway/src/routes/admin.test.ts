...
function createAdminService(canReadAdmin = true, canReadPlatformAdmin = false) {
  return {
    ...
    createDomainClaimForUser: vi.fn(),
    reviewDomainClaimForUser: vi.fn(),
    reviewAccessRequestForUser: vi.fn(),
    resetUserMfaForUser: vi.fn(),
    createBreakGlassSessionForUser: vi.fn(),
    updateBreakGlassSessionForUser: vi.fn(),
    updateTenantGovernanceForUser: vi.fn(),
...
describe('admin routes', () => {
  ...
  it('parses admin audit filters and passes them to the service', async () => {
    ...
    try {
      const response = await app.inject({
        method: 'GET',
        url: '/admin/audit?action=workspace.app.launched&level=info&detectorType=secret&traceId=trace-123&runId=run-123&payloadMode=raw&limit=12',
        headers: {
          authorization: `Bearer ${login.data.sessionToken}`,
        },
      });
