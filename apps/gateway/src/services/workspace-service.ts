...
type WorkspaceService = {
  getCatalogForUser(user: AuthUser): WorkspaceCatalog;
};
...
function resolveMemberGroupIds(user: AuthUser): string[] {
...
export function createWorkspaceService(): WorkspaceService {
  return {
    getCatalogForUser(user) {
      const memberGroupIds = resolveMemberGroupIds(user);
      const defaultActiveGroupId = memberGroupIds[0]!;
      ...
      return {
        ...
        quotaUsagesByGroupId: Object.fromEntries(
          memberGroupIds.map(groupId => [
            groupId,
            [tenantQuota, buildGroupQuota(groupId), userQuota],
          ])
        ),
